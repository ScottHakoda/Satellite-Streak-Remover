...
def estimate_streak_profile(image_data, line, profile_half_width=35, n_samples=100, exclude_mask=None):
    """
    Model a streak using a dynamic angle-correcting approach:
    1. First pass: sample cross-sections to establish global shape parameters.
    2. Inter-pass: Fit a linear polynomial to correct angle errors.
    3. Second pass: render the streak using signed geometric distance, 
       with a soft taper at the endpoints to prevent abrupt background gouging.
    """
    ...
    # =========================================================================
    # FIRST PASS: Estimate parameters & track center drift
    # =========================================================================
    trial_alphas, trial_betas, trial_amps = [], [], []
    trial_centers, trial_t_dists = [], [] 

    for t in np.linspace(0.05, 0.95, min(50, int(length))):
        cx = x1 + t * (x2 - x1)
        cy = y1 + t * (y2 - y1)
    ...
    if np.sum(valid_centers) >= 2:
        center_poly = np.polyfit(trial_t_dists[valid_centers], trial_centers[valid_centers], 1)
    else:
        center_poly = [0.0, med_center]

    # =========================================================================
    # SECOND PASS: Vectorized Rendering with Tapered Endpoints
    # =========================================================================
    ...
    # STRICT MASK: The 15-pixel pad has been removed. Model strictly exists within [0, length]
    valid_mask = (t_dist >= 0.0) & (t_dist <= length) & (d_perp_abs <= profile_half_width)

    dynamic_center = np.polyval(center_poly, t_dist[valid_mask])
    fluxes = moffat(signed_d_perp[valid_mask], global_amp, dynamic_center, global_alpha, global_beta)

    # NEW TAPER LOGIC: Smoothly fade out the flux over the last 3 pixels of the line.
    # This prevents artificial sharp cutoffs or background gouging when the streak ends abruptly.
